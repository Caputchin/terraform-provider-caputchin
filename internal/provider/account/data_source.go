...
func (d *accountDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Metadata for the Caputchin account that owns the provider's management token.\n\nIntended for account-level credentials (session cookie or account-PAT). Per ADR-0027, once team-PAT typing ships in the platform, team-PAT callers will be rejected with an upstream authorization error; configurations that may run under a team-PAT should not depend on this data source.",
